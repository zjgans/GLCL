...
class ResNet(nn.Module):
    ...
    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False,clu_avg_pool=True, drop_rate=0.0,
                 dropblock_size=5, num_classes=-1, no_trans=16, embd_size=128,clu_num=64, use_se=False):
        ...
        if self.num_classes > 0:
            ...
            # #
            self.local_identity_head = nn.Sequential(
                nn.Linear(640, 640),
                nn.BatchNorm1d(640),
                nn.ReLU(inplace=True),
                nn.Linear(640, 640),
    ...
    def forward(self, x, is_feat=False,inductive=False,use_clu_loc=False):
        x = self.layer1(x)
        f0 = x
        x = self.layer2(x)
        f1 = x
        ...
        if inductive:
            inv_x = self.inv_head(feat)
            eq_x = self.eq_head(feat)
            return feat,(x,inv_x,eq_x)
